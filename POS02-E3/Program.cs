...
namespace POS02_E3
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            IEnumerable<string> result1 = db.Exams.Select(e => e.Subject).Distinct().ToList();
            ...
            int result2 = db.Exams.Where(e => e.Subject == "E").Select(e => e.Grade).Max();
            ...
            var result3 = db.Students
                .Where(s => s.Schoolclass == "3CHIF")
                .OrderBy(s => s.Schoolclass).ThenBy(s => s.Lastname)
                .Select(s => new { s.Schoolclass, s.Lastname, s.Firstname, ExamsCount = s.Exams.Count() }).ToList();
            Console.WriteLine("Beispiel 3");
            result3.ForEach(s => { Console.WriteLine($"   {s.Schoolclass}: {s.Lastname} {s.Firstname} hat {s.ExamsCount} Prüfungen."); });
            ...
            var result4 = db.Students
                .Where(s => s.Schoolclass == "3BHIF")
                .Select(s => new { s.Schoolclass, s.Lastname, s.Firstname, Subjects = s.Exams.Select(e => e.Subject).Distinct().ToList() })
                .ToList();
            ...
            var result5 = db.Students
                .Where(s => s.Schoolclass == "3AHIF")
                .Select(s => new
                {
                    s.Lastname,
                    s.Firstname,
                    FailedExams = s.Exams.Where(e => e.Grade == 5).Select(e => new { e.Subject, e.Date }).ToList()
                })
                .ToList();
            ...
            var result6 = db.Exams
                .Select(e => e.Examinator).Distinct()
                .Select(examinator => new
                {
                    Examinator = examinator,
                    BestGrade = db.Exams.Where(e => e.Examinator == examinator).Min(e => e.Grade),
                    WorstGrade = db.Exams.Where(e => e.Examinator == examinator).Max(e => e.Grade)
                })
                .ToList();
